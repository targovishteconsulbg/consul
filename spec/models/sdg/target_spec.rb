...
describe SDG::Target do
  ...
  describe "#official_title" do
    it "returns the official title in the desired language" do
      ...
      expect(target.official_title).to start_with "By 2030, eradicate extreme poverty"

      I18n.with_locale(:es) do
        expect(target.official_title).to start_with "Para 2030, erradicar la pobreza extrema"
      end

      target = SDG::Target["17.11"]

      expect(target.official_title).to start_with "Significantly increase the exports of developing countries"

      I18n.with_locale(:es) do
        expect(target.official_title).to start_with "Aumentar significativamente las exportaciones de los países"
      end
    end
  end
  ...
  describe "#title" do
    ...
    context "translation unavailable" do
      ...
      it "returns the official title when the abbreviated title isn't available" do
        keys = { goals: { goal_1: { targets: { target_1_1: { summary: nil }}}}}
